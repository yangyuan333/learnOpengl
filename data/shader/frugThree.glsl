...
void main()
{
    ...
    FragColor = vec4(CalParallelLight(paralleLight,worldNormal,viewDir),0.0);

    for (int i = 0; i < pointLightNums; i++)
    {
        FragColor += vec4(CalPointLight(pointLights[i],worldNormal,viewDir,worldPos),0.0);
    }
    FragColor += vec4(CalSpotLight(spotLight,worldNormal,viewDir,worldPos),0.0);
    FragColor.w = 1.0;
}
